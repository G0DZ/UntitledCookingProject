...
bool SearchParser::getListPart()
{
    ...
    QGumboNode alala = root.getElementById("content").front();
    listPart = new QGumboNode(alala);
    if(listPart == NULL)
        return false;
    return true;
}
...
std::vector<PrevRecipe> SearchParser::parsePrevRecipes()
{
    std::vector<PrevRecipe> recipes;
    //парсим из root
...
std::vector<PrevRecipe> SearchParser::getRecipesList() const
{
    return recipesList;
}

void SearchParser::setRecipesList(const std::vector<PrevRecipe> &value)
{
    recipesList = value;
}
